...
public struct CUPParser {
    ...
    public func parse(_ text: String) throws -> CUPDocument {
        // Unicode + newline hygiene
        let normalized = text
            .precomposedStringWithCanonicalMapping // NFC normalization for consistent diacritics
            .replacingOccurrences(of: "\u{FEFF}", with: "") // strip any stray BOMs
        ...
        for raw in lines {
            let rawLine = raw            // keep Substring for CSV splitter (avoids extra copies)
            ...
            switch section {
                ...
                case .tasks:
                    ...
                    if isTaskMainLine(line) {
                        ...
                        let turnpoints = names.map { name in
                            CUPTurnpoint(waypointName: name, observationZone: nil)
                        }
    ...
    /// Convenience: if you have bytes already, let CUPReader do decoding.
    public func parse(_ data: Data) throws -> CUPDocument {
        let reader = CUPReader() // strict by default
        let read   = try reader.decodeText(data: data) // ReadResult
        return try parse(read.text)                    // use the String
    }
}
...
fileprivate func isTaskMainLine(_ trimmed: String) -> Bool {
    ...
    // Safer heuristic: require at least 3 commas and avoid key=value lines
